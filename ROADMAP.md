...
| 항목 | 상태 |
|------|------|
| 전체 진행률 | 5% (Sprint 0 완료) |
| 현재 Phase | Phase 0 완료 → Phase 1 진행 예정 |
| 완료된 스프린트 | Sprint 0 (2026-02-28) |
...
### 작업 목록
- [ ] **한국투자증권 계정 준비** _(사용자 수동 작업 - deploy.md 참조)_
  - [ ] 한국투자증권 계좌 개설 (비대면)
  - [ ] HTS ID 등록
  - [ ] KIS Developers 서비스 신청
  - [ ] App Key / App Secret 발급
  - [ ] 모의투자 계정 별도 신청
- [ ] **텔레그램 봇 생성** _(사용자 수동 작업 - deploy.md 참조)_
  - [ ] BotFather로 봇 생성
  - [ ] 봇 토큰 발급
  - [ ] 테스트 채팅방 설정 및 Chat ID 확인
- [x] **개발 환경 설정**
  - [ ] Python 3.12+ 설치 확인 _(사용자 수동 작업)_
  - [ ] Node.js 22+ (LTS) 설치 확인 _(사용자 수동 작업)_
  - [ ] Docker Desktop 설치 및 설정 _(사용자 수동 작업)_
...
- [ ] **대시보드 화면 UI** [Must Have] [복잡도: 높음]
  ...
  - 실행된 주문 요약 타임라인
  - 전략별 성과 요약 카드
  - KOSPI/KOSDAQ 지수 미니 차트 (Lightweight Charts)
- [ ] **관심종목 관리 화면 UI** [Must Have] [복잡도: 중간]
  - 종목 검색 입력 (종목코드 또는 종목명)
  - 검색 결과 드롭다운
  - 그룹별 관심종목 목록 (탭 또는 카테고리)
  - 종목별 현재가, 등락률, 주요 지표 카드
  - 종목 추가/삭제 기능
  - 종목별 전략 할당 드롭다운
...
#### Sprint 4 (Week 4): 전략/백테스팅/설정 화면 UI
...
- [ ] **커스텀 전략 빌더 UI** [Could Have] [복잡도: 높음]
  - 지표 선택 드롭다운 (SMA, EMA, RSI, MACD, BB, ATR 등)
  - AND/OR 조건 조합 빌더
  - 파라미터 입력 필드
  - 전략 미리보기 (텍스트 요약)
  - 전략 저장/불러오기
