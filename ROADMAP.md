...
- **현재 진행 단계:** Phase 7 완료 (Sprint 15: 사용자별 전략/백테스트 데이터 격리 완료, 2026-03-03)
- **팀 규모:** 1-2인 소규모 개발팀
...
| 항목 | 상태 |
|------|------|
| 전체 진행률 | Sprint 0~15 완료 |
| 현재 Phase | Phase 7 완료 — Sprint 14(JWT 인증), Sprint 15(데이터 격리) 완료 (2026-03-03) |
...
**2단계: 복사된 전략 관리 기능 보완**
- ✅ 파라미터 설명 표시: `param_key` 기반 한국어 레이블·설명 정적 매핑 (`strategy-detail-panel.tsx`)
- ✅ 전략 이름 변경: `PUT /strategies/{id}/name` 엔드포인트 + 인라인 편집 UI (`strategy-card.tsx`)
- ✅ 전략 삭제: `DELETE /strategies/{id}` 엔드포인트 + 확인 다이얼로그 UI (FK NULL 처리)
- ✅ 버그 수정: 파라미터 폼 미표시 (`param_type` 매핑 수정), 저장 미연동 (API 연결 완료)
