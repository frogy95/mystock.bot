...
## 개요
- **목표:** 한국투자증권 Open API를 활용한 글로벌 퀀트 전략 기반 자동매매 웹 애플리케이션 (KOSPI/KOSDAQ 우선, 글로벌 시장 확장 예정)
- **전체 예상 기간:** 10주 (2026-03-02 ~ 2026-05-10)
- **현재 진행 단계:** Phase 17 완료 (Sprint 28: 전략 알고리즘 보강 — 기존 3종 개선 + 신규 3종 추가, 2026-03-10)
- **팀 규모:** 1-2인 소규모 개발팀
...
| 항목 | 상태 |
|------|------|
| 전체 진행률 | Sprint 0~28 완료 |
| 현재 Phase | Phase 17 완료 — Sprint 28(전략 알고리즘 보강 — 기존 3종 개선 + 신규 3종 추가) 완료 (2026-03-10) |
| 완료된 스프린트 | Sprint 0 (2026-02-28), Sprint 1 (2026-03-01), Sprint 2 (2026-03-01), Sprint 3 (2026-03-01), Sprint 4 (2026-03-01), Sprint 4.1 (2026-03-01), Sprint 5 (2026-03-01), Sprint 6 (2026-03-01), Sprint 7 (2026-03-01), Sprint 8 (2026-03-01), Sprint 9 (2026-03-02), Sprint 10 (2026-03-02), Sprint 11 (2026-03-02), Sprint 12 (2026-03-03), Sprint 13 (2026-03-03), Sprint 14 (2026-03-03), Sprint 15 (2026-03-03), Sprint 16 (2026-03-04), Sprint 17 (2026-03-05), Sprint 18 (2026-03-06), Sprint 19 (2026-03-06), Sprint 20 (2026-03-06), Sprint 21 (2026-03-07), Sprint 22 (2026-03-08), Sprint 23 (2026-03-08), Sprint 24 (2026-03-08), Sprint 25 (2026-03-08), Sprint 26 (2026-03-09), Sprint 27 (2026-03-09), Sprint 28 (2026-03-10) |
...
| 종목 검색 | yfinance (Yahoo Finance 래퍼) | 무료, API 키 불필요, 전세계 시장 검색 (KRX/NYSE/NASDAQ 등) |
| KRX 한국어 매핑 | 정적 JSON 파일 (`backend/data/krx_stocks.json`) | Docker 내부 pykrx 403 우회, 월 1회 수동 갱신으로 충분 |
...
## Phase 10: 설정 페이지 UX 개선 (Sprint 18)
### 목표
설정 페이지의 복잡한 구조를 탭으로 분리하고, 각 폼에 독립적인 저장 기능 및 toast 피드백을 추가하여 사용성을 개선한다.

#### Sprint 18: 설정 페이지 UX 개선 ✅ 완료 (2026-03-06)
- ✅ **탭 구조 도입**: 3개 탭으로 설정 섹션 분리 (API 연동 / 매매 설정 / 알림 & 위험)
- ✅ **저장 버튼 추가**: 텔레그램, 매매시간, 안전장치 폼에 각각 독립적인 저장 버튼 추가
- ✅ **toast 피드백**: 설정 저장 / 자동매매 토글 / 긴급매도 onSuccess에 toast.success 추가
- ✅ **위험 구역 Card 분리**: 긴급 전체 매도 버튼을 빨간 테두리/배경 Card로 분리, w-full 제거
- ✅ **Collapsible 모의투자 키**: 기본 접힌 상태, "선택사항" 라벨 표시 (shadcn/ui Collapsible 신규 추가)
- ✅ **KIS 필드 순서 개선**: 실전투자 키(필수) → 모의투자 키(선택) → HTS ID → 투자 모드
- ✅ **로컬 state 관리**: 매매시간/안전장치/텔레그램 폼이 로컬 state로 변경 사항 관리
- ✅ Playwright UI 검증 7개 시나리오 전부 통과

### 수정 파일
- `frontend/src/app/settings/page.tsx` — 탭 구조 + 위험 구역 Card
- `frontend/src/components/settings/emergency-sell-button.tsx` — w-full 제거
- `frontend/src/components/settings/kis-api-form.tsx` — Collapsible + 필드 순서 변경
- `frontend/src/components/settings/safety-settings-form.tsx` — 로컬 state + 저장 버튼
- `frontend/src/components/settings/telegram-form.tsx` — 로컬 state + 저장 버튼
- `frontend/src/components/settings/trading-time-form.tsx` — 로컬 state + 저장 버튼
- `frontend/src/components/ui/collapsible.tsx` — 신규 (shadcn)
- `frontend/src/hooks/use-settings.ts` — toast 피드백 추가

---

## Phase 14: 백테스트 차트 데이터 DB 캐싱 (Sprint 22)
### 목표
백테스트 실행 시 차트 데이터를 DB에 캐싱하고, KIS API 실패 시 yfinance로 폴백하여 안정적인 데이터 조회를 보장한다.

#### Sprint 22: 백테스트 차트 데이터 DB 캐싱 + yfinance 폴백 ✅ 완료 (2026-03-08)
- ✅ **ChartDataCache ORM 모델** (`backend/app/models/chart_cache.py`): DB 캐시 테이블 정의, symbol/interval/trade_date 복합 인덱스
- ✅ **Alembic 마이그레이션** (`backend/alembic/versions/e5f6a7b8c9d0_chart_data_cache_테이블_추가.py`): chart_data_cache 테이블 생성 마이그레이션
- ✅ **chart_data_service** (`backend/app/services/chart_data_service.py`): DB 캐시 우선 조회 → KIS API → yfinance 폴백 3단계 전략
- ✅ **backtest_engine 연동** (`backend/app/services/backtest_engine.py`): chart_data_service 사용으로 변경
- ✅ **kis_client 개선** (`backend/app/services/kis_client.py`): get_chart() 예외 시 None 반환으로 폴백 가능하도록 수정
- ✅ `pytest 51 passed` (기존 테스트 회귀 없음)

### 수정 파일
| 파일 | 유형 | 설명 |
|------|------|------|
| `backend/app/models/chart_cache.py` | 신규 | ChartDataCache ORM 모델, 복합 인덱스 |
...
## Phase 16: 관심종목 시세 표시 + 백테스팅 UX 개선 (Sprint 26~27)
### 목표
관심종목 추가 시 현재가 0원 버그를 수정하고, 백테스팅 UX를 개선하며, SSE 스트리밍으로 장기 실행 타임아웃 문제를 해결한다.

#### Sprint 26: 개발 프로세스 정책 재정비 ✅ 완료 (2026-03-09)
- ✅ docs/dev-process.md 섹션 정리 및 정책 보강

#### Sprint 27: 관심종목 시세 표시 + 백테스팅 UX 개선 ✅ 완료 (2026-03-09)
- ✅ **백테스팅 종료일 기본값 오늘**: `endDate` 상태 초기값을 `new Date().toISOString().slice(0, 10)`로 설정
- ✅ **관심종목 현재가 0원 버그 수정**: 종목 추가 후 `/api/v1/stocks/{symbol}/quote` REST API 호출하여 즉시 시세 갱신, `useRealtimeQuotes` WebSocket 구독으로 실시간 업데이트
- ✅ **watchlist-store `updateItemQuote` 액션 추가**: symbol 기준 전 그룹의 WatchlistItem 시세 업데이트
- ✅ **백테스팅 종목선택 탭 추가**: "검색 | 보유종목 | 관심종목" 3개 탭, `useHoldings()` / `useWatchlistGroups()` 훅 연동
- ✅ **백테스팅 엔진 O(n²) → O(n) 최적화**: `_build_signals()` 루프에서 `chart_list` 증분 구축 방식으로 변경
- ✅ **SSE 스트리밍 백테스트**: `POST /backtest/run-multi-stream` 엔드포인트, nginx SSE location (proxy_buffering off, timeout 600s), `useBacktestRunMultiSSE()` 훅, `BacktestProgress` 컴포넌트

### 수정 파일 (Sprint 27)
| 파일 | 유형 | 설명 |
|------|------|------|
| `frontend/src/components/backtest/backtest-config-form.tsx` | 수정 | 종료일 기본값 오늘, 종목선택 3탭 |
| `frontend/src/stores/watchlist-store.ts` | 수정 | `updateItemQuote` 액션 추가 |
| `frontend/src/components/watchlist/stock-search.tsx` | 수정 | 종목 추가 후 REST API 시세 갱신 |
| `frontend/src/app/watchlist/page.tsx` | 수정 | WebSocket 구독 + `updateItemQuote` 연동 |
| `backend/app/services/backtest_engine.py` | 수정 | `_build_signals()` O(n) 최적화 |
| `backend/app/api/v1/backtest.py` | 수정 | `POST /backtest/run-multi-stream` SSE 엔드포인트 추가 |
| `docker/nginx/nginx.conf` | 수정 | SSE 전용 location 추가 |
| `frontend/src/hooks/use-backtest.ts` | 수정 | `useBacktestRunMultiSSE()` 훅 추가 |
| `frontend/src/components/backtest/backtest-progress.tsx` | 신규 | SSE 진행상황 표시 컴포넌트 |
| `frontend/src/app/backtest/page.tsx` | 수정 | 다중 전략 실행 시 SSE 방식 + 프로그레스 UI |

---

## Phase 15: 전략 테스팅 업그레이드 (Sprint 23~25)
### 목표
커스텀 전략을 서버에 영속화하고, 다중 전략을 병렬 백테스트하여 랭킹을 매기며, Claude AI로 최적 전략을 자동 추천한다.

#### Sprint 23: 커스텀 전략 백엔드 연동 ✅ 완료 (2026-03-08)
- ✅ **Alembic 마이그레이션**: strategies 테이블에 buy_conditions(JSONB), sell_conditions(JSONB), description(TEXT) 컬럼 추가
- ✅ **Strategy 모델 확장** (`backend/app/models/strategy.py`): buy_conditions, sell_conditions, description 필드 추가
- ✅ **DynamicStrategy 엔진** (`backend/app/services/strategy_engine.py`): JSONB 조건 파싱, pandas-ta 동적 지표 계산, AND/OR 로직 평가
- ✅ **커스텀 전략 API**: POST /strategies/custom (생성), PUT /strategies/{id}/conditions (조건 수정)
- ✅ **프론트엔드 동기화**: useCreateCustomStrategy, useUpdateCustomStrategy 훅 + custom-strategy-editor.tsx 저장 버튼
- ✅ `pytest 51 passed`

#### Sprint 24: 다중 백테스트 + 랭킹 + 전략 매칭 ✅ 완료 (2026-03-08)
- ✅ **다중 백테스트 API** (`POST /backtest/run-multi`): asyncio 병렬 실행, 전략별 결과 취합
- ✅ **랭킹 스코어 계산** (`backend/app/services/backtest_metrics.py`): calculate_ranking_score() — 수익률(30%)+샤프(25%)+승률(20%)+MDD역수(15%)+CAGR(10%) 가중합
- ✅ **종목 상태 확인 API** (`GET /backtest/stock-status/{symbol}`): 보유/관심종목 여부 반환
- ✅ **설정 폼 개선**: 단일 Select → 체크박스 다중 선택 (프리셋/커스텀 그룹 구분)
- ✅ **랭킹 테이블** (`backtest-ranking-table.tsx`): 순위/전략명/수익률/MDD/샤프/승률/스코어/적용 버튼
- ✅ **에쿼티 차트**: 단일 라인 → 다중 라인 오버레이 (전략별 색상)
- ✅ `pytest 51 passed`

#### Sprint 25: AI 전략 추천 ✅ 완료 (2026-03-08)
- ✅ **AI 어드바이저** (`backend/app/services/ai_advisor.py`): claude-haiku-4-5-20251001로 백테스트 성과 분석 + 전략 추천
- ✅ **AI 추천 API** (`POST /backtest/ai-recommend`): 전략별 성과 요약 입력 → 추천 전략/신뢰도/분석/리스크/포지션 조언 반환
- ✅ **AI 추천 컴포넌트** (`backtest-ai-recommendation.tsx`): 신뢰도 뱃지, 리스크 경고, 포지션 조언 카드
- ✅ **useAIRecommend() 훅** + 백테스트 페이지 통합

### 수정 파일 (Sprint 23~25)
| 파일 | 유형 | 설명 |
|------|------|------|
| `backend/alembic/versions/*_커스텀_전략_컬럼_추가.py` | 신규 | strategies 테이블 JSONB 컬럼 마이그레이션 |
| `backend/app/models/strategy.py` | 수정 | buy/sell_conditions, description 필드 추가 |
| `backend/app/services/strategy_engine.py` | 수정 | DynamicStrategy 엔진 구현 |
| `backend/app/api/v1/strategies.py` | 수정 | 커스텀 전략 생성/수정 API |
| `backend/app/api/v1/backtest.py` | 수정 | run-multi, stock-status, ai-recommend 엔드포인트 추가 |
| `backend/app/services/backtest_metrics.py` | 수정 | calculate_ranking_score() 추가 |
| `backend/app/services/ai_advisor.py` | 신규 | Anthropic API 기반 AI 전략 어드바이저 |
| `backend/app/schemas/backtest.py` | 수정 | 다중 백테스트/AI 추천 스키마 추가 |
| `backend/requirements.txt` | 수정 | anthropic 패키지 추가 |
| `frontend/src/components/backtest/backtest-ranking-table.tsx` | 신규 | 전략 랭킹 테이블 |
| `frontend/src/components/backtest/backtest-ai-recommendation.tsx` | 신규 | AI 분석 카드 |
| `frontend/src/components/strategy/custom-strategy-editor.tsx` | 수정 | 서버 저장 버튼 추가 |
| `frontend/src/hooks/use-strategy.ts` | 수정 | 커스텀 전략 CRUD 훅 추가 |
| `frontend/src/hooks/use-backtest.ts` | 수정 | useAIRecommend(), useMultiBacktest() 훅 추가 |

---

## Phase 13: 전략/백테스트 버그 수정 (Sprint 21)
### 목표
전략 엔진의 한글 전략명 매핑 오류를 수정하고, 앱 시작 시 프리셋 전략을 자동 생성하며, 백테스트 폼에서 API 기반 전략 목록 및 종목 검색을 연동한다.

#### Sprint 21: 전략/백테스트 버그 수정 ✅ 완료 (2026-03-07)
- ✅ **전략 이름 매핑 수정** (`backend/app/services/strategy_engine.py`): `_NAME_TO_ENGINE` 딕셔너리 추가, DB 한글 전략명 → 레지스트리 영문 키 매핑, `get_strategy()` 한글 이름 지원
- ✅ **프리셋 전략 자동 생성** (`backend/app/main.py`): `_PRESET_STRATEGIES` 상수 정의, `ensure_preset_strategies()` 함수, lifespan에서 앱 시작 시 자동 호출
- ✅ **백테스트 폼 개선** (`frontend/src/components/backtest/backtest-config-form.tsx`): mockStrategies 제거 → `useStrategies()` API 연동, 종목코드 직접 입력 → `useStockSearch()` Popover 기반 검색
- ✅ `pytest 51 passed` (기존 테스트 회귀 없음)
...
| 파일 | 유형 | 설명 |
|------|------|------|
| `backend/app/services/strategy_engine.py` | 수정 | `_NAME_TO_ENGINE` 매핑 딕셔너리 추가, `get_strategy()` 한글 이름 지원 |
...
## Phase 12: GHCR 이미지 기반 CI/CD 전환 (Sprint 20)
### 목표
`docker-compose.prod.yml`의 `build:` 방식을 제거하고 GHCR 이미지 기반으로 전환하여, CD 워크플로우가 `docker compose pull && up -d`만으로 30초 내 자동 배포되도록 개선한다.

#### Sprint 20: GHCR 이미지 기반 CI/CD 전환 ✅ 완료 (2026-03-06)
- ✅ **docker-compose.prod.yml 개선**: 모든 `build:` 블록 제거, nginx `image: ghcr.io/frogy95/mystock-bot-nginx:latest` 추가
- ✅ **nginx GHCR 이미지 push** (`.github/workflows/deploy.yml`): `NGINX_IMAGE` env 추가, nginx 빌드+push 스텝 추가
- ✅ **SCP compose 파일 전송** (`.github/workflows/deploy.yml`): `appleboy/scp-action`으로 `docker-compose.prod.yml` 서버 자동 전송
- ✅ **CI nginx 빌드 검증** (`.github/workflows/ci.yml`): nginx 이미지 빌드 확인 스텝 추가
- ✅ **CI/CD 정책 문서화** (`docs/ci-policy.md`): nginx 이미지 테이블, SCP 방식 CD 절차, 롤백 절차 업데이트
- ✅ CI 통과 (pytest + Docker 빌드 3종 모두 통과)

### 수정 파일
| 파일 | 유형 | 설명 |
|------|------|------|
| `docker-compose.prod.yml` | 수정 | `build:` 3개 제거, nginx `image:` 추가 |
| `.github/workflows/deploy.yml` | 수정 | `NGINX_IMAGE` env, nginx 빌드+push, SCP 전송 스텝 추가 |
| `.github/workflows/ci.yml` | 수정 | nginx 이미지 빌드 검증 스텝 추가 |
| `docs/ci-policy.md` | 수정 | nginx 이미지, SCP 방식, 롤백 절차 문서화 |

---

## Phase 11: 글로벌 종목 검색 구현 (Sprint 19)
### 목표
pykrx KRX API 403 에러로 동작하지 않던 종목 검색을 yfinance 기반으로 전환하고, 한국 종목 한국어 이름 매핑 및 한글 IME 입력 버그를 수정한다. 향후 글로벌 시장 확장의 기반을 마련한다.

#### Sprint 19: 글로벌 종목 검색 구현 ✅ 완료 (2026-03-06)
- ✅ **yfinance 종목 검색 서비스** (`backend/app/services/stock_search.py`)
  - `yf.Search(query, max_results=N)` → 전세계 시장 실시간 검색
  - KRX 심볼(`.KS`/`.KQ`) 감지 → 접미사 제거 → 한국어 이름 매핑
  - 글로벌 종목: exchange 필드를 market으로 그대로 반환
  - Redis 5분 TTL 캐시 (`stock_search:{query}`)
  - `asyncio.to_thread`로 블로킹 I/O 비동기 처리
- ✅ **KRX 한국어 종목명 매핑 서비스** (`backend/app/services/krx_names.py`)
  - `backend/data/krx_stocks.json` → 앱 시작 시 인메모리 dict 로드
  - `get_korean_name(code)` / `get_market(code)` 빠른 조회
  - JSON 파일 없어도 영문명으로 우아하게 폴백
- ✅ **정적 KRX 데이터** (`backend/data/krx_stocks.json`)
  - KOSPI/KOSDAQ 주요 종목 ~180개 시드 데이터 포함
  - `scripts/update_krx_stocks.py`로 전 종목 갱신 가능 (로컬 전용)
- ✅ **pykrx 런타임 의존 제거** (`backend/requirements.txt`)
  - `pykrx>=1.0.0`, `setuptools>=65.0.0` 제거
  - `yfinance>=0.2.0` 추가
- ✅ **lifespan 변경** (`backend/app/main.py`): `load_stock_master` → `load_krx_names`
- ✅ **stock_master.py 단순화**: pykrx 코드 제거, stock_search로 위임
- ✅ **한글 IME 버그 수정** (`frontend/src/components/watchlist/stock-search.tsx`)
  - Popover `onOpenAutoFocus` → `e.preventDefault()` (Sprint 18에서 이미 적용)
  - 300ms 디바운스 추가 (`useEffect` + `setTimeout`)
- ✅ **타입 확장** (`frontend/src/lib/mock/types.ts`)
  - `Stock.market`, `WatchlistItem.market`: `"KOSPI" | "KOSDAQ"` → `string` (글로벌 시장 대응)
  - `stock-search.tsx`에서 `as "KOSPI" | "KOSDAQ"` 타입 캐스팅 제거
- ✅ `pytest 51 passed` (기존 테스트 회귀 없음)
- ⬜ yfinance 실제 검색 동작 확인 — Docker 재빌드 필요 (`docker compose up --build`)

### 수정 파일
| 파일 | 유형 | 설명 |
|------|------|------|
| `backend/data/krx_stocks.json` | 신규 | KRX 한국어 종목 정적 데이터 (시드) |
| `scripts/update_krx_stocks.py` | 신규 | KRX 전 종목 갱신 스크립트 (로컬 전용) |
| `backend/app/services/krx_names.py` | 신규 | 인메모리 KRX 이름 매핑 서비스 |
| `backend/app/services/stock_search.py` | 신규 | yfinance 검색 + KRX 매핑 + Redis 캐시 |
| `backend/app/services/stock_master.py` | 수정 | pykrx 제거, stock_search 위임 래퍼 |
| `backend/app/api/v1/stocks.py` | 수정 | import 경로 변경 |
| `backend/app/main.py` | 수정 | lifespan: load_krx_names로 변경 |
| `backend/requirements.txt` | 수정 | pykrx/setuptools 제거, yfinance 추가 |
| `frontend/src/lib/mock/types.ts` | 수정 | market 타입: `"KOSPI"\|"KOSDAQ"` → `string` |
| `frontend/src/components/watchlist/stock-search.tsx` | 수정 | 300ms 디바운스, 타입 캐스팅 제거 |
...
- ✅ 글로벌 종목 검색 기반 구축 완료 (Sprint 19): yfinance로 NYSE/NASDAQ 등 전세계 종목 검색 가능
- ⬜ 해외 주식 실거래 지원 (미국 주식 KIS API 연동)
...
| Sprint 18 | 2026-03-06 | Phase 10 | 설정 페이지 UX 개선 (탭 구조, 저장 버튼, toast 피드백, 위험 구역 Card) |
| Sprint 19 | 2026-03-06 | Phase 11 | 글로벌 종목 검색 (yfinance 전환, KRX 한국어 매핑, 한글 IME 버그 수정, 300ms 디바운스) |
| Sprint 20 | 2026-03-06 | Phase 12 | GHCR 이미지 기반 CI/CD 전환 (docker-compose.prod.yml build 제거, nginx GHCR push, SCP 파일 전송) |
| Sprint 21 | 2026-03-07 | Phase 13 | 전략/백테스트 버그 수정 (한글 전략명 매핑, 프리셋 자동 생성, 백테스트 폼 API 연동) |
| Sprint 22 | 2026-03-08 | Phase 14 | 백테스트 차트 데이터 DB 캐싱 + yfinance 폴백 (ChartDataCache 모델, chart_data_service, Alembic 마이그레이션) |
| Sprint 23 | 2026-03-08 | Phase 15 | 커스텀 전략 백엔드 연동 (JSONB 조건, DynamicStrategy 엔진, 커스텀 전략 API, 프론트 동기화) |
| Sprint 24 | 2026-03-08 | Phase 15 | 다중 백테스트 + 랭킹 + 전략 매칭 (asyncio 병렬 실행, 랭킹 스코어, 종목 상태 API, 랭킹 테이블/차트) |
| Sprint 25 | 2026-03-08 | Phase 15 | AI 전략 추천 (claude-haiku, ai-recommend API, BacktestAIRecommendation 컴포넌트) |
