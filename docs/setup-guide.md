...
프로젝트를 시작하기 전에 사용자가 직접 수행해야 하는 외부 서비스 준비 작업을 안내합니다.
...
python3 --version  # Python 3.12.x 이상 확인
```
...
`your_*_here` 형태의 플레이스홀더가 모두 실제 값으로 교체되었는지 확인하세요.
...
```bash
# 1. Docker 빌드 및 서비스 기동
docker compose up --build -d

# 2. DB 마이그레이션
docker compose exec backend alembic upgrade head

# 3. 시드 데이터 삽입 (admin 유저 + 전략 3개)
...
> **관리자 계정**: `frogy95@gmail.com` (seed.py로 생성)
> **KIS API 키**: 앱 실행 후 `/settings` 페이지에서 DB로 관리 (Sprint 12 이후)
